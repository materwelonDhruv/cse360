...
public class UserNameRecognizer {
	 ...
	 * Machine
	 * diagram into an executable Java program using the UserName Recognizer. The
	 * code
	 * detailed design is based on a while loop with a select list
	 * </p>
	 * 
	 * <p>
	...
	public static String checkForValidUserName(String input) {
		...
		while (running) {
			...
			switch (state) {
				case 0:
					...
					) // Removed the 0-9 check because first character has to be alphabetic
					{
						nextState = 1;
				...
				case 1:
					...
					// ., _, - -> State 2
					else if ("._-".indexOf(currentChar) >= 0) { // Check for ., _, -
						nextState = 2;
		...
		switch (state) {
			...
			case 2:
				...
				userNameRecognizerErrorMessage += "A UserName character after ., -, _ must be A-Z, a-z, 0-9.\n";
				return userNameRecognizerErrorMessage;
